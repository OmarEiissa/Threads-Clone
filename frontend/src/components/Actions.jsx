...
const Actions = ({ post }) => {
  ...
  const handleLikeAndUnLike = async () => {
    ...
    try {
      const res = await fetch(
        `${VITE_API_BASE_URL}/api/posts/like/${post._id}`,
        {
          method: "put",
          headers: {
      ...
      if (!liked) {
        ...
        // remove the id of the current user from likes array
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
  ...
  const handleReply = async () => {
    ...
    try {
      const res = await fetch(
        `${VITE_API_BASE_URL}/api/posts/reply/${post._id}`,
        {
          method: "put",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <Flex flexDir={"column"}>
      <Flex my={2} onClick={(e) => e.preventDefault()}>
        {/* Liked */}
        <Flex
          alignItems={"center"}
